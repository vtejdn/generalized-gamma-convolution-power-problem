\section{The infinitesimal effect of taking a power}
\label{sec:tangent}

We compute the derivative at $q=1$ of the logarithmic Laplace derivative of $X^q$.
We will use this derivative to construct the evolution in the following sections.

Throughout this section $U$ is a positive measure on $(0,\infty)$ with
\begin{equation}
...
\]
For $s>0$, let $X_s$ be a random variable with the exponentially tilted law $\Law(X_s)(\dd x)=e^{-sx}\Law(X)(\dd x)/L(s)$.
...
\begin{lemma}[Marginal tilts and infinitesimal deterministic powers]
\label{lem:tangent}
...
\emph{Step 1: differentiating the power parameter.}
...
\emph{Step 2: evaluating the tilted logarithmic moment.}
Differentiating the gamma integral with respect to its positive shape parameter, we obtain $\E[G_B\log G_B]=B\psi_0(B+1)$.
Consequently
\[
 \E[X_s\log X_s]
 =B\E\left[M_P(s)\{\psi_0(B+1)+\log M_P(s)\}\right].
\]
\emph{Step 3: differentiating the Dirichlet mean.}
...
We have kept the law of $X$ fixed throughout this calculation.
Later we apply the lemma separately to the GGC distribution specified by the Thorin measure at each time.
The calculation therefore does not presuppose closure under powers.
