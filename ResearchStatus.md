...
Status: **active research problem in this repository**. Last checked:
**2026-09-05**.

## Target

Bondesson's Conjecture 1 is
...
- **[PROJECT STATUS]** We therefore do not treat (C1) as an established
  theorem. The goal is to prove it independently, repair a valid part of the
  claimed argument, or find a counterexample.

The current project ledger has not resolved the full conjecture. It now
does, however, settle \(q=2\) in five substantial regimes:
...
- three arbitrary positive scales with total shape \(1<B<2\), provided the
  shapes carried by the two extreme scales are at most one; WIP-5.39 proves
  this by full-interval Abel monotonicity.  Thus a possible unique shape
  above one may be carried by the middle scale.

For the first nonquadratic benchmark, it also proves a genuinely open
neighborhood of equal rates:
...
This WIP-1.17 theorem is nonconstructive in \(\theta_*\) and does not
settle arbitrary rate contrast.
...
and WIP-5.29 propagates it to every smaller positive total shape.  It does
not settle finite supports of three or more rates when the total shape is
greater than one, and it does not settle the full power conjecture.
...
The remaining finite-gamma square problem has at least three rates and
total shape \(B>1\), outside the proved slices above. WIP-5.34 is the first
...
obstructions is a GGC counterexample; the general one-dimensional Appell
boundary phase remains open.
...
one at the middle scale.  The cases in which that unique large shape is
attached to an extreme scale remain open.
...
positivity.  Arbitrary contrast remains the exact open phase problem.
...
WIP-1.19 rewrites the remaining arcsine average as an even convolution on
...
singular boundary.  The next target is therefore a genuine Abel boundary-
compensation inequality for the full average.
...
The case \(n=1\) follows from HCM-density power closure. Integer gamma shapes
are covered by the \(PF_\infty\) case.  The former first test, \(n=2\) with
arbitrary positive nonintegral shapes, unequal rates, and \(q=2\), is now
closed by WIP-5.30.  For \(q=2\) the first unresolved finite-gamma case has
at least three rates and total shape greater than one; for two rates the
first unresolved powers have \(q\ne2\).
...
### Phase A: secure the base cases

- Prove or disprove the \(n=2\) finite-gamma case for arbitrary
  \(\beta_1,\beta_2,b_1,b_2>0\) and \(q>1\).
- Separate what follows from existing HCM-density and \(PF_\infty\) results
  from what genuinely uses a new kernel identity.
- Test every proposed integral lemma against bounded-Laplace-transform
  specializations like the audit certificate above.

### Phase B: construct a valid positive kernel

- Start from \(\psi_{n,q}(sv)\psi_{n,q}(s/v)\) and seek an explicit positive
  mixture in \(v+v^{-1}\).
- Track parameter-dependent representing measures as kernels rather than as
  fixed measures.
- Audit Tonelli/Fubini, Jacobians, complete-monotonicity composition, and all
  claimed Bernstein functions at each elimination step.

### Phase C: compare alternative analytic routes

- Direct HCM proof for the Laplace transform.
- Stieltjes representation of \(\Phi_q'\) and construction of a Thorin
  measure.
- Mellin-transform or logarithmic coordinates, while keeping deterministic
  dependence visible.
- Bondesson's Conjecture 2 or a weaker integral statement still sufficient
  for finite gamma convolutions.

### Phase D: close the limiting argument

- Pass from finite gamma convolutions to arbitrary GGC laws by weak closure.
- Treat atoms, drift, degeneracy at zero, and parameter endpoints explicitly.
- Record each verified component as a lemma with exact provenance and
  dependencies.

Numerical work may be used to discover a candidate kernel or counterexample,
but the project requires an analytic certificate.
...
| GGC-C1 | every GGC is closed under deterministic powers \(q\ge1\) | Bondesson, Conjecture 1 | open project target |
...
| GGC-N2 | \(S_2^q\in GGC\) for two arbitrary gamma summands when \(q>1,\ q\ne2\) | current project | active target; \(q=1\) is trivial and the entire \(q=2\) family is now GGC-P17 |
...
| GGC-P21 | there exists \(\theta_*>0\) such that \((e^\theta G_{1/2}+e^{-\theta}G'_{1/2})^{3/2}\in GGC\) whenever \(\lvert\theta\rvert<\theta_*\) | WIP-1.17 | proved here by a Kummer ODE, a positive one-rate Wronskian, and a uniform logarithmic diagonal tube; the threshold is not explicit and arbitrary contrast remains open |
...
| GGC-R5 | every three-rate square transform has the exact Lauricella/Appell representations (5.347)--(5.349), reducing to WIP-5.30 on a two-rate boundary | WIP-5.33 | proved exact reduction; the open target is the boundary phase along the correlated one-dimensional Appell curve |
...
| GGC-R7 | for three rates and \(0<B<2\), \(G_{B/2}M^2\in GGC\) is equivalent to monotonic decrease of the Abel ratio \(B_{B/2}(x)/A_{B/2}(x)\) on both sides of the middle knot plus the nonnegative one-sided phase-jump condition (5.396) | WIP-5.36--WIP-5.39 | proved exact physical-curve reduction and all node signs; WIP-5.39 closes both interval interiors when the two extreme-scale shapes are at most one, while a shape above one at an extreme scale remains open |
| GGC-R8 | in the reciprocal-scale half-gamma \(q=3/2\) model, the physical Wright-ray average is the even logarithmic convolution (1.153), satisfies the radial Klein--Gordon equation (1.154), and has the separation-current representation (1.160) | WIP-1.19 | proved exact all-contrast reduction; the required current sign remains open, and direct separation integration by parts has the wrong bulk sign and a singular boundary |
